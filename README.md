...
## What is OBS Tally?
OBS tally is an android overlay application that allows cameramen to see if their camera is live on OBS Studio

I am using [RTSP Camera Server](https://play.google.com/store/apps/details?id=com.miv.rtspcamera) as a RTSP media source for OBS Studio,
...
## Disclamer
This have been tested on MacOS, I do not know if it works with Windows.
...
* Before building the Android app, create a `amqp.properties` file based on `amqp.properties.sample` and replace <username> and <password> by the corresponding values in the file
